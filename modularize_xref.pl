    ...
    find_in_source(File,
                    [Term, _Info, Result]>>(
                        ( ( Term = (:- module(_, PredsOrOps));
                          ...
                          include(=(op(_, _, _)), PredsOrOps, Result)
                        )
                    ),
                    Ops0),
    append(Ops0, Ops1),
...
%
%  If the file at =Path= doesn't already import =Predicates= from
%  =Module=, add the corresponding use_module/2 declaration. If it
%  already does, add to the import list if needed.
add_use_if_needed(Path, Module, Predicates) :-
    ...
    % Also import operator if needed
    ( module_operator_exports(Module, ModuleOperators) -> true ; ModuleOperators = [] ),
    sort(Predicates, PredsSet),
    include({PredsSet}/[op(Prec, Type, Name)]>>(
                \+ ord_memberchk(op(Prec, Type, Name), PredsSet),
                ( Type = fx
                -> ord_memberchk(Name/1, PredsSet)
                ; ord_memberchk(Name/2, PredsSet) )
            ),
            ModuleOperators, IncludedModuleOperators),
    append(Predicates, IncludedModuleOperators, PredsAndOperators),
    insert_use_module(Path, Module, PredsAndOperators, UseModuleEnd).
...
module_operator_exports(Module, OpExports) :-
    xref_module(ModulePath, Module),
    find_in_source(
        ModulePath,
        [(:- module(_, Exports)), _, OpEx]>>include(=(op(_,_,_)), Exports, OpEx),
        OpExports0),
    sort(OpExports0, OpExports).
...
break_loop_by_merging(Loop, Mod1, Mod2) :-
    ...
    setup_call_cleanup(open(NewModulePath, write, S, []),
                       ( formatted_module(NewModule, NewModuleExports, NewModuleHeaderString),
                         write(S, NewModuleHeaderString),
                         write(S, ".\n\n"),
                         write(S, Mod1Content),
                         write(S, Mod2Content)
                       ),
                       close(S)),
    ( Mod1Path \= NewModulePath -> delete_file(Mod1Path) ; true ),
    ( Mod2Path \= NewModulePath -> delete_file(Mod2Path) ; true ),
    maplist([termpos_exports(_, _, Exports), Exports]>>true, Mod1ModuleLocExports, Mod1Exports0),
    append(Mod1Exports0, Mod1Exports),
    change_other_imports(Mod1, NewModule, Mod1Exports),
    maplist([termpos_exports(_, _, Exports), Exports]>>true, Mod2ModuleLocExports, Mod2Exports0),
...
break_loop_by_inlining(Loop, InlineToMod, DefiningMod, AllToImportExtract) :-
    ...
    once(( member(InliningToPath, Loop), file_module(InliningToPath, InlineToMod) )),
    selectchk(DefiningMod-ExtractPreds, AllToImportExtract, AllToImport),
    debug(xxx, "INLINING ~q FROM ~q into ~q", [ExtractPreds, ThisModPath, InliningToPath]),
    copy_predicates_into_caller(InliningToPath, ThisModPath, ExtractPreds),
    % either also import AllToImport or inline those too?
    forall(( member(Mod-Preds, AllToImport), Mod \= InlineToMod),
           add_use_if_needed(InliningToPath, Mod, Preds)).
...
copy_predicates_into_caller(CallerModuleFile, CalleeModuleFile, PredIndicators) :-
    debug(xxx, "COPY ~q FROM ~q INTO ~q", [PredIndicators, CalleeModuleFile, CallerModuleFile]),
    find_in_source(
        CalleeModuleFile,
    ...
    setup_call_cleanup(open(CallerModuleFile, append, S),
                       ( seek(S, 0, eof, _),
                         forall(member(PredContent, ExtractedPreds),
                                % TODO: don't put newlines in between same predicate
    ...
    forall(member(Imports-Info, ModuleLocs),
           ( get_dict(subterm_positions, Info, Pos),
             ...
             ( ( Remainder == [] ; forall(member(X, Remainder), X = op(_, _, _)) )
             -> true
